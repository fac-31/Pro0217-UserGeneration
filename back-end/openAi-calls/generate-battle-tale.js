...
async function generateBattleTale(winner, loser) {
	...
	try {
		...
		if (winner && loser && battleType) {
			...
			prompt = `"You are a children's book writer who specializes in spy novels. Write a short spy face-off in a living room that describes a battle of ${battleType}, but dont mention the battle type specifically. There needs to be a bit of action, a battle of strength, but the winner ultimatley wins through the battle type. ${winner.name} fights ${loser.name} with ${winner.weapon} against ${loser.weapon}. Keep the tone light and funny, but easy to understand. Use ${winner.name}'s biography (${winner.biography}) to add the winners personality to the story but don't mention specific things that we already know about the character - use this to infer the way that they might behave. ${winner.name} always wins. Keep the battle under 125 words, using short, simple sentences. If no name is entered for either character, create one. End by saying who won the face-off. Correct any grammatical errors and split any words that are joined without spaces."`;
		} else {
